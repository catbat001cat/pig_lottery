...
class YmfpayController extends HomebaseController
{

    private $wx_pay_db = null;
    
    private $url = 'http://gateway.pooulcloud.cn/paygate/merchant_info/pay';
    ...
    public function entry()
    {
        ...
        $new_sign = $this->sign($params_url, C('YMF_MCH_KEY'));
    ...
    public function MakeSign($params)
    {
        ...
        $logHandler= new \CLogFileHandler("logs/ymf_".date('Y-m-d').'.log');
        $log = \Log::Init($logHandler, 15);
        
        //签名步骤二：在string后加入KEY
        $string = $string . "&key=".C('YMF_MCH_KEY');
    ...
    public function pay($order, $goback)
    {
        ...
        $params['merchant_id'] = C('YMF_MCHID');
        $params['mch_trade_id'] = $order_sn;
        $params['subject'] = '充值' . $price . '元';
        $params['body'] = '充值' . $price . '元';
        $params['total_fee'] = $price * 100;
        $params['spbill_create_ip'] = get_client_ip();
        $params['notify_url'] = "http://" . $_SERVER['HTTP_HOST'] . "/api/ymfpay/notify_wx2312_458671";
        ...
        \Log::DEBUG ( 'YmfpayController支付调用开始:' . $params ['mch_trade_id'] );
        ...
        if ($respJson ['code'] == '0' && $respJson ['msg'] == 'SUCCESS') {
        	redirect ( $respJson['data']['code_url'] );
