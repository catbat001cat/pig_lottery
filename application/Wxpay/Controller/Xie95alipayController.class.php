...
class Xie95alipayController extends HomebaseController {
	...
	public function entry() {
		...
		$new_sign = $this->sign ( $params_url, C ( 'XIE95_DESKEY2' ) );
		if ($new_sign != $sign) {
	...
	public function pay($order, $pay_goback) {
		...
		$params ['version'] = '1.7';
		$params ['channel'] = 'alipayQR';
		$params['settleCycle'] = '0';
		$dest_key = '';
		$params ['mchId'] = C ( 'XIE95_MCHID2' );
		$dest_key = C ( 'XIE95_DESKEY2' );
		$params ['body'] = '充值' . $price . '元';
		$params ["outTradeNo"] = $order_sn;
		$params ["amount"] = $price;
		$params ['notifyUrl'] = "http://" . $_SERVER ['HTTP_HOST'] . "/api/xie95alipay/notify_wx2312_458671";
		...
		\Log::DEBUG ( 'Xie95alipayController支付调用开始:' . $params ['mchId'] );
