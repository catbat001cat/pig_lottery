...
class BcfwxpayController extends HomebaseController {
	private $wx_pay_db = null;
	private $url = 'http://qr.bcfapi.com/unite/v4.0/payment';
	...
	public function entry() {
		...
		require_once SITE_PATH . "/wxpay/log.php";
		$logHandler = new \CLogFileHandler ( "logs/bcf_" . date ( 'Y-m-d' ) . '.log' );
		$log = \Log::Init ( $logHandler, 15 );
		...
		$new_sign = $this->sign ( $params_url, C ( 'BCF_MCH_KEY' ) );
		if ($new_sign != $sign) {
	...
	public function pay($order, $pay_goback) {
		...
		$params = array ();
		$params ['merchantNo'] = C('BCF_MCHID');
		$params ['source'] = "0";
		$params ['subject'] = '充值' . $price . '元';
		$params ['orderNo'] = $order_sn;
		$params ['orderMoney'] = $price * 100;
		$params ['notifyUrl'] = "http://" . $_SERVER ['HTTP_HOST'] . "/api/bcfpay/notify_wx2312_458671";
		
		$params ['sign'] = $this->MakeSign ( $params );
		
		$jsonStr = json_encode ( $params );
		
		\Log::DEBUG ( 'BcfwxpayController支付调用开始' );
		...
		if ($respJson ['status'] == 'SUCCESS') {
			$this->assign('qrcode', $respJson['payUrl']);
		} else {
