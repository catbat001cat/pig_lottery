...
class Xie95alipayController extends HomebaseController {
	...
	private function MakeSign($params)
	{
	    ...
	    //签名步骤二：在string后加入KEY
	    $string = $string . "&key=".C('XIE95_DESKEY2');
	...
	public function notify_wx2312_458671() {
            ...
            */
            ...
            \Log::DEBUG("Xie95alipayController,:" . $server_ip . ',channel:' . $channel . ',mchId:' . $mchId . ',outTradeNo:' . $outTradeNo . ',outChannelNo:' . $outChannelNo . ',amount:' . $amount . ',transTime:' . $transTime . ',status:' . $status . ',sign:' . $sign);
            
            if (!empty(C('XIE95_SERVER_IP')))
            {
            	if (C('XIE95_SERVER_IP') != $server_ip)
                {
                    \Log::DEBUG('Xie95alipayController: ip地址不正确:' . $server_ip . ',' . $outTradeNo . ',' . $amount);
                    return;
                }
            }
            ...
            if ($params['sign']!= $sign)
            {
            	\Log::DEBUG('Xie95alipayController: key不正确:' . $server_ip . ',' . $outTradeNo . ',' . $amount);
