...
class YmfpayController extends HomebaseController {
	...
	public function notify_wx2312_458671() {
            ...
            $mchId= $params['data']['merchant_id'];
            $outTradeNo= $params['data']['mch_trade_id'];
            $amount = intval($params['data']['total_fee']) / 100.0;
            $transTime = $params['data']['time_end'];
            $outChannelNo = $params['data']['ch_trade_id'];
            $sign = $params['sign'];
            
            //$params['sign'] = $this->MakeSign($params);

            if (!empty(C('YMF_SERVER_IP')))
            {
            	if (C('YMF_SERVER_IP') != $server_ip)
                {
                    \Log::DEBUG('YmfpayController: ip地址不正确:' . $server_ip . ',' . $outTradeNo . ',' . $amount);
                    return;
                }
            }
        ...
        private function deal_order2312($order_sn, $transition_id, $total_fee)
        {
            ...
            \Log::DEBUG('YmfpayController:' . $order_sn . ',' . $transition_id. ',' . $total_fee);
