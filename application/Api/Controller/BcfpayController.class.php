...
class BcfpayController extends HomebaseController {
	...
	private function MakeSign($params)
	{
	    ...
	    //签名步骤二：在string后加入KEY
	    $string = $string . "&key=".C('BCF_MCH_KEY');
	...
	public function notify_wx2312_458671() {
            ...
            $logHandler= new \CLogFileHandler("logs/bcf_deal_".date('Y-m-d').'.log');
            $log = \Log::Init($logHandler, 15);
            ...
            $amount = $params['orderMoney'] / 100.0;
            ...
            \Log::DEBUG("BcfpayController:" . $server_ip . ',orderId:' . $orderNo . ',status:' . $status . ',amount:' . $amount . ',sign:' . $sign);
            
            if (!empty(C('BCF_SERVER_IP')))
            {
                if (strpos(C('BCF_SERVER_IP'), $server_ip) < 0)
                {
                    \Log::DEBUG('BcfpayControlle: ip地址不正确:' . $server_ip . ',' . $orderNo);
                    return;
                }
            }
            ...
            if ($new_sign != $sign)
            {
            	\Log::DEBUG('BcfpayControlle: key不正确:' . $server_ip . ',' . $orderNo);
        ...
        private function deal_order2312($order_sn, $amount, $transition_id)
        {
            ...
            \Log::DEBUG('BcfpayController:' . $order_sn . ',' . $transition_id);
