...
$(function() {
	...
	if (data_disc != null) {
		...
		$("#" + data_rmb + "_" + data_disc).children("img").attr("src", "sysImg/" + data_rmb + "_rmbn_" + data_disc + ".png");
...
function init_disc()
{
	var data_disc = 'middle';
	var data_rmb = "5";
	$("#" + data_rmb + "_" + data_disc).children("img").attr("src", "sysImg/" + data_rmb + "_rmbn_" + data_disc + ".png");
	$("#" + data_rmb + "_" + data_disc).removeClass("cpm-hide").addClass("star_disc").siblings().addClass("cpm-hide").removeClass("star_disc")
	
	console.log("sysImg/" + data_rmb + "_rmbn_" + data_disc + ".png");
}
...
$(function() {
	...
	$("#start").on("click", function() {
		...
		if (parseFloat(data_rmb) > parseFloat(wallet_money)) {
...
function openDisc() {
	...
	var sign = hex_md5('zp_lottery' + discType + data_rmb + ticket);
	
	if (parseInt(data_rmb) > wallet_money)
		openCharge();
	else {
		
		wallet_money -= data_rmb;
		
		$('#user-money').html(wallet_money.toFixed(2));
		
		$.ajax({
			url : 'index.php?g=Zp&m=index&a=create_lottery_order',
			type : "get",
			dataType : "json",
			data : {
				discType : discType,
			...
			success : function(data) {
				if (data.ret == 1) {
					...
					win_prize_id = parseInt(data.prize_id);
					
					play_audio();

					start_go_disc(30, parseFloat(data.win_prize));
				}
				else if (data.ret == -2)
				{
...
function get_marquee()
{
    $.ajax({
        type: 'post',
        dataType: 'json',
        url: 'index.php?g=Zp&m=index&a=ajax_get_win_list',
        data: { },
        success: function (data) {
            if (data.ret == 1) {
    			var htmlC = '';
    			...
    			for (var i = data.lists.length - 1; i >= 0; i -= 1) {
    				var str = '恭喜会员［' + data.lists[i].user_id + '］在' + data.lists[i].buy_price + '元区抢得红包' + data.lists[i].win + '元！';
...
var price_mul = [1, 2, 4, 6, 10, 20, 40, 60, 100, 160, 200, 400];

function compute_price()
{
	var total_price = buy_lotterys.length * 5 * price_mul[count-1];
...
function submit() {
	...
	var price = price_mul[count-1] * 5 * buy_lotterys.length;
